...
export async function renderModalForm(TE: TemplateBike) {
    const collection = await TE.render('modal/window');
    const modalWindow = <Element> collection.childNodes[0];
    await TE.render('modal/header', modalWindow);
    const signFormCollection = await TE.render('forms/sign_form');
    const form = <Element> signFormCollection.childNodes[0];
